...
MODE=${MODE:-release}
OUT=${OUT:-"${ROOT}/build/fs_smoke.elf"}
MANIFEST="${ROOT}/apps/fs_smoke/Cargo.toml"
LINKER_SCRIPT="${ROOT}/apps/fs_smoke/linker.ld"
...
APP_BIN="${CARGO_TARGET_DIR}/${TARGET}/${MODE}/fs_smoke"
if [[ ! -f "${APP_BIN}" ]]; then
  echo "fs_smoke binary not found: ${APP_BIN}" >&2
  exit 1
fi

mkdir -p "$(dirname "${OUT}")"
cp "${APP_BIN}" "${OUT}"
echo "Built fs_smoke ELF: ${OUT}"
