...
MODE=${MODE:-release}
OUT=${OUT:-"${ROOT}/build/tcp_echo.elf"}
MANIFEST="${ROOT}/apps/tcp_echo/Cargo.toml"
LINKER_SCRIPT="${ROOT}/apps/tcp_echo/linker.ld"
...
APP_BIN="${CARGO_TARGET_DIR}/${TARGET}/${MODE}/tcp_echo"
if [[ ! -f "${APP_BIN}" ]]; then
  echo "tcp_echo binary not found: ${APP_BIN}" >&2
  exit 1
fi

mkdir -p "$(dirname "${OUT}")"
cp "${APP_BIN}" "${OUT}"
echo "Built tcp_echo ELF: ${OUT}"
