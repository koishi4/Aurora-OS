...
MODE=${MODE:-release}
OUT=${OUT:-"${ROOT}/build/shell.elf"}
MANIFEST="${ROOT}/apps/shell/Cargo.toml"
LINKER_SCRIPT="${ROOT}/apps/shell/linker.ld"
...
APP_BIN="${CARGO_TARGET_DIR}/${TARGET}/${MODE}/shell"
if [[ ! -f "${APP_BIN}" ]]; then
  echo "shell binary not found: ${APP_BIN}" >&2
  exit 1
fi

mkdir -p "$(dirname "${OUT}")"
cp "${APP_BIN}" "${OUT}"
echo "Built shell ELF: ${OUT}"
